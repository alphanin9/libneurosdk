		...
		switch (*str) {
			...
			case '\"':
				...
				*dst++ = '\"';
				break;
			default:
				// Note: this does not escape \u2028 and \u2029 so it's not fully JSONy
...
neurosdk_context_send(neurosdk_context_t *ctx, neurosdk_message_t *msg) {
	...
	switch (msg->kind) {
		...
		case NeuroSDK_MessageKind_ActionsForce: {
			char const *query = msg->value.actions_force.query;
			if (!query) {
				LOG_ERROR(context, "actions/force: 'query' is required but is NULL.");
				return NeuroSDK_InvalidMessage;
			}
			char const **action_names = msg->value.actions_force.action_names;
			...
			char const *ephemeral_context_str = "null";
			if (!ephemeral_null) {
